...
**Q: What does `NA` mean?** **A:** `NA` represents a **missing value** (unknown/not recorded).
...
**Why `color`/`fill` are mapped but `alpha` is a geom argument?** `color`/`fill` reflect **data-driven aesthetics** (continent varies by row), so they belong in `aes()`.
`alpha` here is a **constant stylistic choice** applied uniformly, so it’s set on the geom (outside `aes`) rather than mapped to data.
...
-   **Violins** reveal **shape** (multi-modality, tail thickness) hidden by box plots.
-   **Box plots** emphasize **median, IQR, and conventional outliers**; clearer for compact comparisons across groups.
-   Using both offers complementary insight: use violins to see structure; box plots to summarize.
...
**Relationship (direct):** Generally **positive**: more per-capita generation corresponds to more mismanaged waste per person.
The **spread** indicates heterogeneous waste-management efficiency across countries.
...
**Which appears more linear?** Neither is strongly linear; after log scaling, **coastal population** sometimes exhibits a **slightly tighter** relationship than total population, but signal remains **weak**.
Consumption behavior and infrastructure likely dominate over sheer population counts.
...
Plastic waste vs coastal population proportion
